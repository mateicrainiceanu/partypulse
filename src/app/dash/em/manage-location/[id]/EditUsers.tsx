...
function EditUsers({id}: {id: string}) {
	...
	useEffect(() => {
  	...
  	function deleteUser(uid: number) {
			axios.delete("/api/location/" + id + "/users/" + uid).then(response=>{setUsers(response.data)});
		}
		...
		<div className="w-full">
          ...
          variant="filled"
			...
			<table className="w-full text-center ">
				...
				<tbody className="overflow-y-scroll max-h-10">
					{users
							...
							<tr key={user.id} className="h-8">
								...
								<td className="text-center ">
									<i className="text-red-400 hover:text-red-500 ">
										<BsXCircleFill onClick={()=>{deleteUser(user.id)}}/>
									</i>
								</td>
							</tr>
						))}
